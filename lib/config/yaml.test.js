...
describe('loadYAMLConfig', () => {
  ...
  test('throws an error if the config can not be loaded', async () => {
    ...
    await expect(loadYAMLConfig(path)).rejects.toThrow(/Failed to load YAML config due to/)
  })

  test('throws an error if the loaded config is invalid', async () => {
    const path = await tempWrite('foo bar baz')

    await expect(loadYAMLConfig(path)).rejects.toThrow(/Failed to load YAML config due to: Loaded config is not an object/)
  })
})
