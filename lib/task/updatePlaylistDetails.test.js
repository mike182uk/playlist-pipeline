...
import { id, getConfigSchema, execute } from './updatePlaylistDetails.js'
...
describe('execute', () => {
  ...
  test('updates the name of a playlist', async () => {
    ...
    expect(spotifyChangePlaylistDetailsStub).toHaveBeenCalledTimes(1)
    expect(spotifyChangePlaylistDetailsStub).toHaveBeenCalledWith(playlistId, { name })
  })
  ...
  test('throws an error if the playlist details can not be updated', async () => {
    ...
    ).rejects.toThrow('an error occurred updating playlist details: failed to update playlist details')
  })
})
