...
export async function execute ({ config, spotify, trackCollections }) {
  ...
  try {
    ...
    if (clearTracksStatusCode !== 201) {
      throw new Error(`unexpected status code returned (${clearTracksStatusCode}) whilst clearing tracks in playlist`)
    }
    ...
    for (const trackUrisChunk of chunkedTrackUris) {
      ...
      if (statusCode !== 201) {
        throw new Error(`unexpected status code returned (${statusCode}) whilst replacing tracks in playlist`)
      }
    }
  } catch (err) {
