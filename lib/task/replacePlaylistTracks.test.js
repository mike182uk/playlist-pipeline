...
const { id, getConfigSchema, execute } = require('./replacePlaylistTracks')
...
describe('execute', () => {
  ...
  it('throws an error if the tracks in the playlist could not be cleared', async () => {
    ...
    ).rejects.toThrow('Invalid status code returned [500] whilst clearing tracks in playlist')
  })
  ...
  it('throws an error if the tracks can not be added to the playlist', async () => {
    ...
    ).rejects.toThrow('Invalid status code returned [500] whilst replacing tracks in playlist')
  })
})
