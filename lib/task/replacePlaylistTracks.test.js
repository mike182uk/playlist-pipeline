...
import { execute, getConfigSchema, id } from "./replacePlaylistTracks.js"
...
describe("execute", () => {
  it("replaces the tracks in a playlist", async () => {
    ...
    const spotifyReplaceTracksInPlaylistStub = vi.fn().mockResolvedValue({
      statusCode: 201,
    })
    const spotifyAddTracksToPlaylistStub = vi.fn().mockResolvedValue({
      statusCode: 201,
    })
    ...
    await execute({
      ...
      trackCollections: {
        [trackCollectionName]: [{ uri: "abc123" }, { uri: "abc456" }],
      },
    })

    expect(spotifyReplaceTracksInPlaylistStub).toHaveBeenCalledTimes(1)
    expect(spotifyReplaceTracksInPlaylistStub).toHaveBeenCalledWith(
      playlistId,
      []
    )
