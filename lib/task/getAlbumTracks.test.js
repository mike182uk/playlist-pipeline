...
describe('execute', () => {
  test('retrieves the album tracks', async () => {
    ...
    decorateTracksWithArtistGenres.mockImplementation(() => Promise.resolve(decorateTracksWithArtistGenresResult))
    ...
    expect(decorateTracksWithArtistGenres).toBeCalledWith(
      getTracksRecursivelyResult.map((track) => {
        return { track }
      }),
      spotify
    )
