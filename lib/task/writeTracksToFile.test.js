...
const {
  ...
  execute,
  FORMAT_JSON
} = require('./writeTracksToFile')
const { writeFile } = require('fs')

test('has correct id', () => {
  expect(id).toBe('tracks.write_to_file')
})
...
describe('execute', () => {
  test('writes tracks to file', async () => {
  ...
  test('throws an error if an error occurred whilst writing to file', async () => {
    const trackCollectionName = 'foo'
